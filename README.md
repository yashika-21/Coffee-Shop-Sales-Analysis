...
```
**Q4. Revenue generated by each category?**
```sql
SELECT PRODUCT_CATEGORY,ROUND(SUM(TRANSACTION_QTY * UNIT_PRICE)) 'SALES' FROM SALES GROUP BY PRODUCT_CATEGORY ORDER BY ROUND(SUM(TRANSACTION_QTY * UNIT_PRICE)) DESC;
```
